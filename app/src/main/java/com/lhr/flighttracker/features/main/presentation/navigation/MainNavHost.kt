...
import com.lhr.flighttracker.features.map.presentation.ui.IndoorMapScreen
...
fun MainNavHost(navController: NavHostController) {
    NavHost(navController = navController, startDestination = Screen.Flight.route) {
        ...
        composable(route = Screen.IndoorMap.route) {
            IndoorMapScreen()
        }

        composable(route = Screen.Settings.route) {
            SettingsScreen(LocalNavController.current)
        }
    }
}